...
def repo_file(repo, *path, mkdir=False):
    """Same as repo_path, but create dirname if absent.  For
    example, repofile(r, \"refs\", \"remotes\", \"origin\", \"HEAD\") will create
...
def repo_create(path):
    """Create a new repoistory at path."""
...
whatever it takes to convert it into a meaningful representation. What exactly that means depends on the subclass"""
...
def object_hash(fd, fmt, repo=None):
    ...
    match fmt:
        ...
        case _: raise Exception("Uknown type %s!" % fmt)
...
def kvlm_parse(raw, start=0, dct=None):
    if not dct:
        ...
        # You CANNOT declare the object as dct=OrderedDict() or all
    ...
    # Recursive case
    # ==============
    # We read a key-value pair and reurse for the next.
    key = raw[start:spc]
...
def log_graphviz(repo, sha, seen):
    ...
    short_hash = sha[0:8]
    message = commit.klvm[None].decode("utf8").strip()
    ...
    if not b'parent' in commit.klvm.keys():
...
# value, which is compare using the default rules.  So we just return
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'taggger'] = b'Wyag <wyag@example.com>'
        # ...and a tag message!
...
def ref_create(repo, ref_name, sha):
    with open(repo_file(repo, "refs/" + ref_name, sha), 'w') as fp:
        fp.write(sha + "\n")
